...
echo '<h3>View Orders</h3><table class="table table-responsive table-sm" border="0" width="100%" cellspacing="4" cellpadding="4" id="orders">
<thead>
	<tr>
...
$q = 'SELECT o.id, FORMAT(total/100, 2) AS total, c.id AS cid, CONCAT(last_name, ", ", first_name) AS name, city, state, zip, COUNT(oc.id) AS items, t.charge_id FROM orders AS o
 ...
 JOIN charges AS t ON (t.order_id=o.id AND t.type="auth_only") GROUP BY o.id, t.charge_id DESC';
