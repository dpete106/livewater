...
$page_title = 'Vermont Farm Stand at Livewater';
...
UNION (SELECT CONCAT('C', sc.id), CONCAT('$', FORMAT(sa.price/100, 2)), sa.ship, gc.category, gc.image, CONCAT_WS(' - ', s.size, sc.caf_decaf, sc.ground_whole) FROM sales AS sa INNER JOIN specific_coffees AS sc ON sa.product_id=sc.id INNER JOIN sizes AS s ON s.id=sc.size_id INNER JOIN general_coffees AS gc ON gc.id=sc.general_coffee_id WHERE sa.product_type='cand' AND ((NOW() BETWEEN sa.start_date AND sa.end_date) OR (NOW() > sa.start_date AND sa.end_date IS NULL) ) ORDER BY RAND() LIMIT 2)
UNION (SELECT CONCAT('M', sc.id), CONCAT('$', FORMAT(sa.price/100, 2)), sa.ship, gc.category, gc.image, CONCAT_WS(' - ', s.size, sc.caf_decaf, sc.ground_whole) FROM sales AS sa INNER JOIN specific_coffees AS sc ON sa.product_id=sc.id INNER JOIN sizes AS s ON s.id=sc.size_id INNER JOIN general_coffees AS gc ON gc.id=sc.general_coffee_id WHERE sa.product_type='maple' AND ((NOW() BETWEEN sa.start_date AND sa.end_date) OR (NOW() > sa.start_date AND sa.end_date IS NULL) ) ORDER BY RAND() LIMIT 2)
";
